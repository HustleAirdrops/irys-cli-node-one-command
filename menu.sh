...
def download_videos(query, output_file, target_size_mb=1000, max_filesize=1100*1024*1024, min_filesize=1*1024*1024):
    ydl_opts = {
        'format': 'best',
        'noplaylist': True,
        'quiet': True,
    ...
    try:
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            if not candidates:
                print("⚠️ No suitable videos found (at least 1MB and up to ~1GB).")
                return
            ...
            if not downloaded_files and candidates:
                size, v = candidates[0]  # smallest
                current_file = 1
                print(f"🎬 Downloading large video to trim {current_file}: {v['title']} ({format_size(size)})")
                ydl.download([v['webpage_url']])
                filename = ydl.prepare_filename(v)
                if os.path.exists(filename) and os.path.getsize(filename) > 0:
                    downloaded_files.append(filename)
            ...
            if not downloaded_files:
                print("⚠️ No videos found close to target size.")
                return
